...
export const POST = async (req: NextRequest) => {
  try {
    ...
    const user = await db.user.findUnique({
      ...
      // include: { EventCategories: true }, //so this joined 2 tables for the spefic user of that apikey and get all the event categories
    });
    ...
    const hasNotificationChannel =
      user.discordId || user.telegramId || user.whatsappId;
    if (!hasNotificationChannel) {
      return NextResponse.json(
        {
          message:
            "Please configure at least one notification channel (Discord, Telegram, or WhatsApp) in your account settings",
        },
        { status: 403 },
      );
    }
    ...
    //adding a redis layer here
    const quotaLimit =
      user.plan === "FREE"
        ? FREE_QUOTA.maxEventsPerMonth
        : PRO_QUOTA.maxEventsPerMonth;

    const redisKey = `quota:${user.id}:${currentYear}:${currentMonth}`;

    const cachedCount = await redis.get<number>(redisKey);

    if (cachedCount != null) {
      if (cachedCount >= quotaLimit) {
        return NextResponse.json(
          {
            message:
      ...
      try {
        ...
        //first we have to set data then send because it will return;
        const count = quota?.count ?? 0;
        await redis.set(redisKey, count);
        await redis.expire(redisKey, 60 * 60 * 24 * 35);
