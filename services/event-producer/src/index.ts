  ...
  await kafka.subscribe(
    ...
    async ({ message }) => {
      ...
      try {
        ...
        const description =
          event.description ||
          `A new ${event.category.name} event has occurred!`;
        ...
        // Adding a redis based caching here
        const redisKey = `user:${event.userId}`;

        const cachedUser = await redis.get<UserNotificationChannels>(redisKey);
        let user = null;
        if (cachedUser != null) {
          user = cachedUser;
        } else {
          try {
            ...
            await redis.expire(redisKey, 60); // 1 minute
          } catch (error) {
            logger.error("error in retriving data from database for the user");
          }
        }
